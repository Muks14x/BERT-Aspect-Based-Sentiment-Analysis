...
def readfiles(words_file, labels_file):
    '''
    reads words and labels
    '''
    fwords = open(words_file)
...
class DataProcessor(object):
    ...
    @classmethod
    def _read_dataset(cls, words_file, labels_file):
        """Reads a words file and a labels file."""
        return readfiles(words_file, labels_file)
...
def evaluate(args, processor, model, tokenizer, device, eval_on):
    if eval_on == "dev":
...
def main():
    ...
    processors = {"term_extraction":TermExtractionProcessor}
    ...
    model = TermExtraction.from_pretrained(args.bert_model,
              from_tf = False,
              config = config)
    ...
    if args.do_train:
        ...
        for e_no in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            valid_report, valid_f1 = evaluate(args, processor, model, tokenizer, device, "dev")
            test_report, test_f1 = evaluate(args, processor, model, tokenizer, device, "test")
        ...
        model = TermExtraction.from_pretrained(args.output_dir)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        # Load the best model
        model = TermExtraction.from_pretrained(args.output_dir_bestmodel)
